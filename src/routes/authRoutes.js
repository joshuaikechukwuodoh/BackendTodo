...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const existingUser = await db.prepare('SELECT * FROM user WHERE username = ?').get(username);
        if (existingUser) {
            return res.status(409).json({ error: 'Username already exists' });
        }

        const hashedPassword = await bcrypt.hash(password, 10);
        const result = await db.prepare('INSERT INTO user (username, password) VALUES (?, ?)')
...
router.post('/login', async (req, res) => {
    ...
    try {
        const user = await db.prepare('SELECT * FROM user WHERE username = ?').get(username);
        if (!user) {
